...
export default function ResultCard({ item, index, maxScore }) {
      ...
      <div className="flex">
        <div className="relative w-20 sm:w-24 flex-shrink-0 overflow-hidden">
          <img
            ...
            alt={item.code}
            className="w-full h-full min-h-[120px] object-cover transition-transform duration-500 group-hover:scale-110"
            onError={e => { e.target.src = FALLBACK_IMG; }}
          />
          <div className="absolute inset-0 bg-gradient-to-l from-stone-900/60 to-transparent" />
        </div>

        <div className="flex-1 p-4 flex flex-col justify-between min-h-[120px]">
          <div>
            <p className="font-mono text-sm font-bold text-stone-100 tracking-wide mb-3">
              {item.code}
            </p>
            <div className="space-y-1.5">
              <div className="flex items-center justify-between text-[10px] text-stone-500">
                <span>התאמה</span>
                <span className="font-mono font-semibold text-amber-500/80">{pct}%</span>
              </div>
          ...
          <a
            ...
            className="inline-flex items-center gap-1.5 mt-3 px-3 py-1.5 text-[11px] font-medium text-amber-500/80 border border-amber-800/25 rounded-lg hover:bg-amber-900/20 hover:text-amber-400 transition-all self-start"
          >
            פרטים באתר
